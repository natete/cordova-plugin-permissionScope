...
@objc open class PermissionScope: UIViewController, CLLocationManagerDelegate, UIGestureRecognizerDelegate, CBPeripheralManagerDelegate {
    ...
    /// Denied alert config
    open var deniedAlertTitle:String?
    open var deniedAlertMessage:String?
    open var deniedCancelActionTitle:String?
    open var deniedDefaultActionTitle:String?
    /// Disabled alert config
    open var disabledAlertTitle:String?
    open var disabledAlertMessage:String?
    open var disabledCancelActionTitle:String?
    open var disabledDefaultActionTitle:String?
    ...
    var motionPermissionStatus: PermissionStatus = PermissionStatus.Unknown
    ...
    func requiredAuthorized(_ completion: (Bool) -> Void ) {
        getResultsForConfig{ results in
          let result = results.first(where: { $0.status != .Authorized }).isNil
            completion(result)
        }
    }
    ...
    public init(backgroundTapCancels: Bool) {
        ...
        closeButton.addTarget(self, action: #selector(PermissionScope.cancel), for: .touchUpInside)
